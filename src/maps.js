...
function transitionSum(a) {
    let lo = clamp01(a);
    let hi = 1;
    const evaluate = (s) => (s ** 4) - (s * s) + a * s - a * a;
    if (evaluate(hi) <= 0) {
        return hi;
    }
    for (let i = 0; i < 64; i++) {
...
function lowCoverageRoot(localC) {
...
function cell2Coverage(a, localC) {
    if (localC <= EPS) {
        return null;
    }
    if (isNear(localC, 1)) {
...
function cell5Coverage(a, localC) {
    if (localC <= EPS) {
        return null;
    }
...
function cell6Coverage(a, localC) {
    if (localC <= EPS) {
        return null;
    }
    const disc = Math.max(0, a * a - a * localC + localC * localC);
...
function admissible(aInput, bInput, localCInput) {
...
function fallbackCoverage(a, localC, seed) {
...
export function maxAdmissibleCoverage(aInput, localCInput) {
    ...
    const candidates = [0, a, 1 - a, circleArcBound(a), transitionSum(a) - a];
